...
namespace NBeam {
    ...
    struct History {
        ...
        Flips flips;
        HistoryPtr parent;
        History(const Operation& op_, const Flips& flips_, HistoryPtr parent_)
            : op(op_), flips(flips_), parent(parent_) {}
    };
    ...
    struct Stack {
        ...
        Stack push(const Operation& op, const Flips& flips) {
            return Stack({ std::make_shared<History>(op, flips, head) });
        }
        Stack pop() {
    ...
    struct Node {
        ...
        void advance(const Operation& op, Flips& flips) {
            state.apply_move(op, flips);
        }
    };
    ...
    Node beam_search(const State& initial_state, const int beam_width) {
        ...
        for (int turn = 1;; turn++) {
            ...
            for (int i = 0; i < std::min(beam_width, node_size); i++) {
                ...
                Flips flips;
                next_nodes.back().advance(temp_nodes[i].op, flips);
                next_nodes.back().move_history = nodes[node_index].move_history.push(temp_nodes[i].op, flips);
            }
